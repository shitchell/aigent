...
class AigentApp(App[None]):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.args = args
        self.ws_url = f"ws://{args.host}:{args.port}/ws/chat/{args.session or 'tui-session'}?client_type=tui"
    ...
    async def on_mount(self) -> None:
        try:
            ...
            self.query_one("#chat", ChatContainer).add_message("Connected to Aigent.", role="system")
        except Exception as e:
            self.query_one("#chat", ChatContainer).add_message(f"Connection Failed: {e}", role="system")
        ...
        try:
            async for message in self.ws:
                ...
                if event_type == "token":
                    ...
                    await self.push_screen(ApprovalDialog(
