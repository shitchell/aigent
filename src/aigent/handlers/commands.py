...
async def on_client_input_command_check(message: Message, session: Session, user: User) -> None:
    ...
    await bus.dispatch(specific_event, args=args, session=session, user=user, message=message)
...
async def on_help(session: Session) -> None:
    ...
    await bus.dispatch(CoreSignal.SYSTEM_OUTPUT, content=msg, session=session)
