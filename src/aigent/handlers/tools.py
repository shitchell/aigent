...
async def on_approval_resolved(request_id: str, decision: str, session: Session) -> None:
...
async def _execute_tool(request: ToolRequest, session: Session) -> None:
    ...
    try:
        ...
        await bus.dispatch(
            ToolSignal.EXECUTE_ERROR, request_id=request.request_id, error=str(e), session=session
        )
